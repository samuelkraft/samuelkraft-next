...
og: '/blog/ios-chat-bubbles-css/og.png'
...
Let's recreate these shapes with our pseudo elements. Start by creating the colored shape with a big border (`border-right: 20px`). We save a few lines of css by using border over setting width/height and background-color separately. Set the curve using border-radius, we can mimic the iOS one using different values for the horizontal and vertical axis. Then tweak the `right` value until it's positioned correctly.
...
.sent {
  ...
  &:after {
    right: -26px;
    border-right: 26px solid var(--bg);
