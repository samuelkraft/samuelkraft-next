...
og: '/blog/spring-parallax-framer-motion-guide/og.png'
---
...
## I'm convinced - How to implement parallax spring easing in react?
...
Next up we'll import `useViewportScroll` from framer-motion in order to get how far on the page we have scrolled. We'll use this as an input to `useTransform` in order to calculate our parallax values which we'll add to a `motion.div`.
...
const Parallax = ({ children }: ParallaxProps): JSX.Element => {
  ...
  const y = useTransform(scrollY, [100, 200], [500, -500])
