...
__Update:__ Thanks to reader Chris Morgan for pointing out that we need to reverse our properties and start with the fallback, as no browser that supports `@supports` supports `mix-blend-mode`.
