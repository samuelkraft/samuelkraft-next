...
__Update:__ Thanks to reader Chris Morgan for pointing out that we need to reverse our properties and start with the fallback, as no browser that doesn't support `mix-blend-mode` supports `@supports`.
