...
<Alert>
  ...
  backdrop and is useful for coloring monochrome images or tinting color images.
</Alert>
...
**Update:** Thanks to reader Chris Morgan for pointing out that we need to reverse our properties and start with the fallback, as all browsers that don’t support `mix-blend-mode` also don’t support `@supports`.
