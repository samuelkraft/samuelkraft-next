...
async function generate() {
  await Promise.all(
    routes.map(async (route) => {
      ...
      const poly = polyline
        .fromGeoJSON(geoJson.features[0].geometry)
        .replace(/\?/g, "%3F");
      ...
      context.fillText(
        `${Math.round(route.distance * 10) / 10} km`,
        distanceX,
        titleY
      );
      context.fillText(`${Math.round(route.elevation)} m`, elevationX, titleY);
      context.fillText(
        `${Math.round(route.elevation / route.distance)}`,
        stifaX,
        titleY
      );
      ...
      fs.writeFileSync(
        path.join(process.cwd(), `public/og/${route.slug}.png`),
        buffer
      );
    })
  );
...
<Alert>
  It took me a while to figure out that the api does not accept polylines
  containing question marks "<strong>?</strong>" as that breaks the url
  parameters. Be sure to replace all instances with it's encoded counterpart.
</Alert>
...
In the code below I am adding a gradient _before_ I add the text, that way the gradient will be positioned beneath the text in the final image.
